...
class ForumController extends AbstractController
{
    ...
    ): Response
    {
        ...
        if ($response = $this->renderCreateValidationError($titre, $contenu, $categorie, $request, $postRepository, $categorieForumRepository, $bookmarkRepository, $reactionPostRepository)) {
            return $response;
        }
    ...
    private function renderForumIndexPage(
        ...
        $commentCounts = $postRepository->findCommentCountsByPostIds($postIds);
        $reactionCounts = $postRepository->findReactionCountsByPostIds($postIds);
        $reactionTotals = $this->buildReactionTotals($reactionCounts);
        $userReactionTypes = $reactionPostRepository->findUserReactionTypesByPostIds($this->getForumUser(), $postIds);
        ...
        return $this->render('user/forum/index.html.twig', [
            ...
            'favorite_bookmarked_ids' => $bookmarkFlags['favorite'],
            'saved_bookmarked_ids' => $bookmarkFlags['saved'],
            'post_comment_counts' => $commentCounts,
            'post_reaction_counts' => $reactionCounts,
            'post_reaction_totals' => $reactionTotals,
            'post_user_reaction_types' => $userReactionTypes,
            'forum_active_section' => 'feed',
            'search' => $search,
            'sort' => $sort,
            'currentPage' => $page,
    ...
    private function renderForumBookmarksPage(
        ...
        $reactionCounts = $postRepository->findReactionCountsByPostIds($postIds);
        $reactionTotals = $this->buildReactionTotals($reactionCounts);
        $userReactionTypes = $reactionPostRepository->findUserReactionTypesByPostIds($user, $postIds);
        $translatedPosts = $this->buildTranslatedForumPosts($posts, $locale);
     ...
     */
    private function buildReactionTotals(array $reactionCounts): array
    {
        $totals = [];

        foreach ($reactionCounts as $postId => $counts) {
            $totals[(int) $postId] = array_sum($counts);
        }
