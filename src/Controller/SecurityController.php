...
class SecurityController extends AbstractController
{
    ...
    /**
     * GET — show a confirmation button page (safe for email scanner pre-fetch).
     * The actual confirmation only happens on POST, so scanners cannot trigger it.
     */
    #[Route('/participation/{id}/confirmer/{token}', name: 'public_participation_confirm', requirements: ['id' => '\d+'], methods: ['GET'])]
    public function confirmParticipationGet(int $id, string $token): Response
    {
        ...
        // Show the confirmation button — do NOT confirm yet.
        return $this->render('security/confirmation.html.twig', [
            'status'      => 'pending',
            'eventName'   => $eventName,
            'confirmUrl'  => $this->generateUrl('public_participation_confirm_post', [
                'id'    => $id,
                'token' => $token,
            ]),
        ]);
    ...
    /**
     * POST — actually confirms the participation and sends the ticket email.
     * Only a real browser submitting the form reaches this route.
     */
    #[Route('/participation/{id}/confirmer/{token}', name: 'public_participation_confirm_post', requirements: ['id' => '\d+'], methods: ['POST'])]
    public function confirmParticipationPost(int $id, string $token): Response
    {
        $participation = $this->participationService->getById($id);
        if (!$participation) {
            return $this->render('security/confirmation.html.twig', [
