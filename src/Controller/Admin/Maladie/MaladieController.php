...
class MaladieController extends AbstractController
{
    ...
    private MaladieRepository $maladieRepo;
    private SolutionTraitementRepository $traitementRepo;
    ...
    public function new(Request $request, SluggerInterface $slugger): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            $nomScientifique = trim((string) $request->request->get('nomScientifique', ''));
            $description     = trim((string) $request->request->get('description', ''));
            $symptomes       = trim((string) $request->request->get('symptomes', ''));
            $niveauGravite   = (string) $request->request->get('niveauGravite', 'moyen');
            $saisonFrequente = (string) $request->request->get('saisonFrequente', '');
            ...
            if (empty($errors)) {
                ...
                $maladie->setNiveauGravite($niveauGravite ?: null);
                ...
                if ($imageFile) {
                    ...
                    try {
                        ...
                        $imageFile->move((is_string($projectDir) ? $projectDir : '') . '/public/uploads/maladies', $newFilename);
    ...
    public function edit(Maladie $maladie, Request $request, SluggerInterface $slugger): Response
    {
        ...
        return $this->render('admin/maladie/edit.html.twig', [
            'maladie'     => $maladie,
            'traitements' => $maladie->getId() !== null ? $this->traitementRepo->findByMaladieId($maladie->getId()) : [],
            'errors'      => [],
        ]);
    }
    ...
    {
        if ($this->isCsrfTokenValid('delete' . $maladie->getId(), (string) $request->request->get('_token'))) {
    ...
    public function newSolution(Maladie $maladie, Request $request): Response
    {
        ...
        $etapes            = trim((string) $request->request->get('etapes', ''));
        $produitsRecommandes = trim((string) $request->request->get('produitsRecommandes', ''));
        $conseilsPrevention  = trim((string) $request->request->get('conseilsPrevention', ''));
        $dureeTraitement   = trim((string) $request->request->get('dureeTraitement', ''));
    ...
    public function deleteSolution(SolutionTraitement $traitement, Request $request): Response
    {
        ...
        if ($this->isCsrfTokenValid('delete-solution' . $traitement->getId(), (string) $request->request->get('_token'))) {
